...
- Total JIRI memory: target under 350MB.
...
- AI cache lookup on the render path: under 5ms.
- AI API timeout: max 8 seconds, background worker only.
...
- AI requests must be background only and must never be reachable from the render path.
