...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
    ...
    try:
        ...
        print(f"Port {port} is already in use; admin was not started.", file=sys.stderr)
        print("Stop the existing process or run with JIRI_ADMIN_PORT=<free-port>.", file=sys.stderr)
        sys.exit(1)
PY
